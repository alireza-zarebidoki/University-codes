...
این سند مطابق با الزام بخش تحویلی‌های صورت پروژه تهیه شده و شامل دو بخش اصلی است: نحوه‌ی اجرای پروژه، و توضیح کامل معماری، کلاس‌ها و فیلدهای پیاده‌سازی‌شده.
...
این دستور ۹۳ بررسی خودکار روی تمام ماژول‌های سیستم (بدون نیاز به ورودی دستی) اجرا می‌کند و در انتها خلاصه‌ی نتایج را چاپ می‌کند. جزئیات کامل نحوه‌ی تست (خودکار و دستی) در فایل جداگانه‌ی `TESTING.md` آمده است.
...
### ساخت و مشاهده‌ی مستندات JavaDoc

مستندات فنی کامل کلاس‌ها در پوشه‌ی `doc/` (کنار `src/`) قرار می‌گیرد. برای ساخت آن از ریشه‌ی پروژه:
...
برای مشاهده، کافی است فایل `doc/index.html` را در هر مرورگری باز کنید.
 `-private` عمداً اضافه شده تا فیلدها و متدهای private هم (نه فقط API عمومی) در مستندات دیده شوند، چون هدف این‌جا توضیح کامل پیاده‌سازی است، نه فقط یک API عمومی برای مصرف‌کنندگان بیرونی.
...
├── SelfTest.java                      مجموعه تست خودکار (۹۳ بررسی)
...
├── repository/                        ۴ Repository + SystemStateSnapshot + FilePersistenceManager + CsvExporter
│
├── ui/
...
پوشه‌ی `doc/` (کنار `src/`، نه داخلش) هم مستندات JavaDoc تولیدشده از کامنت‌های همین کلاس‌ها را نگه می‌دارد.
...
### `CsvExporter` (خروجی کمکی، فقط برای خواندن)

علاوه بر فایل باینری اصلی (`hotel_data.ser`)، هر بار ذخیره‌سازی انجام شود، یک نسخه‌ی مکمل و **قابل‌خواندن برای انسان** هم به‌صورت ۸ فایل CSV در پوشه‌ی `csv_export/` نوشته می‌شود: `rooms.csv`, `users.csv`, `reservations.csv`, `service_orders.csv`, `invoices.csv`, `payments.csv`, `maintenance_requests.csv`, `logs.csv`.

نکته‌ی مهم: این خروجی **فقط یک‌طرفه (Export)** است، نه یک مکانیزم جایگزین برای بارگذاری مجدد برنامه. چون فرمت CSV ذاتاً جدولی/مسطح است، نمی‌تواند رابطه‌های بین اشیاء (مثلاً این‌که یک `Guest` داخل `Reservation` دقیقاً همان شیءِ داخل مخزن کاربران است) را حفظ کند. بارگذاری واقعی وضعیت برنامه همچنان فقط از طریق `FilePersistenceManager` (فایل باینری `Serializable`) انجام می‌شود؛ CSV صرفاً برای بازکردن راحت داده‌ها در اکسل/گوگل‌شیت یا بازرسی سریع است.

### `DataInspector` (ابزار مستقل بازرسی داده)

یک کلاس مستقل با `main` جداگانه (در `com.grandpersia.DataInspector`) که با استفاده از همان `FilePersistenceManager`، فایل `hotel_data.ser` را می‌خواند و یک گزارش کامل و خوانا (همه‌ی اتاق‌ها، کاربران، رزروها، فاکتورها، لیست انتظار، درخواست‌های تعمیرات، و لاگ‌ها) در کنسول چاپ می‌کند. برای بازرسی سریع محتوای فایل ذخیره‌سازی، بدون نیاز به اجرای کامل برنامه:
...
### `SelfTest.java`
مجموعه‌ی ۹۳ بررسی خودکار (بدون هیچ کتابخانه‌ی تست خارجی) روی تمام ماژول‌ها؛ جزئیات کامل در `TESTING.md`.

### `DataInspector.java`
نقطه‌ی ورود مستقل دیگر؛ فایل ذخیره‌سازی (`hotel_data.ser`) را می‌خواند و محتوایش را به‌صورت خوانا در کنسول چاپ می‌کند (توضیح کامل در بخش ۱۳).
